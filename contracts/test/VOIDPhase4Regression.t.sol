...
import {ModelPool, ModelPositionManager, ModelSwapRouter02, BaseWETH9, GenerousWETH9, UniMath} from "./UniV3Model.sol";
...
contract Phase4PoCTest is Test {
    ...
    function setUp() public {
        ...
        executor =
            new VOIDGraduationExecutor(IVOIDGraduationCurve(address(curve)), IVOIDGraduationRouter(address(router)));
    ...
    function testC2_DirectGraduationWhenNoCorrectionIsNecessary() public {
        ...
        // The floored seed caps leave a sub-ppb sqrt-price gap. The executor
        // intentionally treats that rounding-only difference as no correction.
        vm.prank(bob);
    ...
    function testC7_DeployGateRejectsAHandlerLessSafe() public {
        ...
        (bool ok, bytes memory ret) =
            bare.staticcall(abi.encodeCall(IERC721Receiver.onERC721Received, (address(this), address(this), 0, "")));
    ...
    // =====================================================================
    // N-01 REGRESSION: benign seed rounding requires no correction capital
    // =====================================================================

    function testN01_ExecutorAcceptsRoundingOnlyGapWithoutCorrectionAssets() public {
        _fundToThreshold();
        vm.prank(safe);
        curve.seedMigrationPool();
        ...
        // No approval and no ETH are required for this rounding-only difference.
        vm.prank(bob);
        executor.execute{value: 0}(0);
        assertTrue(curve.graduated());
    }

    function testN01_RoundingOnlyPathRefundsEthAndNeverPullsApprovedVoid() public {
        _fundToThreshold();
        vm.prank(safe);
        curve.seedMigrationPool();
        ...
        assertTrue(curve.graduated());
        assertEq(bob.balance, ethBefore, "unused ETH returned");
        assertEq(token.balanceOf(bob), voidBefore, "approved VOID never pulled");
    }
    ...
    // =====================================================================
    // N-02 REGRESSION: cached WETH fits the Base WETH9 transfer stipend
    // =====================================================================

    function testN02_ExecutorRefundWorksThroughBaseWethTransferStipend() public {
        _fundToThreshold();
        vm.prank(safe);
        curve.seedMigrationPool();
        assertEq(address(executor.weth9()), address(weth), "WETH cached at construction");

        // A zero-value Base-style withdraw proves receive() fits the 2300 stipend.
        vm.prank(address(executor));
        weth.withdraw(0);

        // Force the correction direction to WETH regardless of token address ordering.
        if (address(token) < address(weth)) {
            _movePoolBy(9_500, 10_000);
        } else {
            _movePoolBy(10_500, 10_000);
        }
        assertFalse(_correctionIsVoid(), "regression must exercise the WETH refund branch");
        _executeWith(bob, 60 ether, 0);
    ...
    function testGas_ControlWithGenerousWeth() public {
        ...
        bool needsVoid =
            (ModelPool(pool).sqrtPriceX96() > e2.targetSqrtPriceX96(address(gw))
