...
/// @title VOIDB20SkinController
/// @notice Competitive burn-to-transform controller for the Base-native VOID B20.
/// @dev The B20 is created adminless. This controller is its only BURN_ROLE and
///      METADATA_ROLE holder, so every metadata update is bound to an approved burn.
contract VOIDB20SkinController is Ownable2Step, ReentrancyGuard {
    ...
    constructor(address initialOwner, IB20 b20Token) Ownable(initialOwner) {
        if (initialOwner == address(0) || address(b20Token) == address(0)) revert InvalidToken();
        token = b20Token;
        renamePaused = true;
    }
    ...
    function controllerReady() public view returns (bool) {
        // A B20 address uses native execution behind marker bytecode; role reads,
        // not bytecode shape, are the authoritative readiness signal.
        return token.hasRole(B20Constants.BURN_ROLE, address(this))
            && token.hasRole(B20Constants.METADATA_ROLE, address(this));
    }

    /// @notice All supply-reducing burns, including contest burns and any other authorized burns.
    ...
    function approveRename(
        ...
        delete _activeSlot;
        token.updateName(proposedName);
        token.updateSymbol(proposedSymbol);
        token.updateContractURI(metadataURI);
