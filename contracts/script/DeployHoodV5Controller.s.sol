...
contract DeployHoodV5Controller is Script {
    ...
    function run() external returns (VOIDHoodSkinController controller) {
        ...
        address safe = vm.envAddress("PRODUCTION_SAFE");
        ...
        require(registry.ownerOf(tokenAddress) == safe, "Safe is not Hood token owner");
        require(token.tokenOwner() == safe, "tokenOwner mismatch");
        ...
        vm.startBroadcast();
        controller = new VOIDHoodSkinController(safe, token, registry);
        vm.stopBroadcast();

        require(controller.owner() == safe, "controller owner mismatch");
        ...
        console2.log("Controller owner:", safe);
