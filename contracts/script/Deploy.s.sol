...
contract DeployVOIDCoin is Script {
    function run() external returns (VOIDCoin token, VOIDTreasuryVesting vestingWallet) {
        ...
        address positionRecipient = vm.envAddress("POSITION_RECIPIENT");
        ...
        require(positionRecipient != address(0), "POSITION_RECIPIENT must be nonzero");
        ...
        VOIDLaunch launch = new VOIDLaunch(
            safe, migrationTarget, positionRecipient, virtualEthReserve, graduationThreshold, initialTokenURI
        );
        vm.stopBroadcast();
        ...
        console2.log("Position recipient:", positionRecipient);
