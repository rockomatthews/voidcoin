...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (process.env.RESEND_API_KEY) {
      ...
      await resend.emails.send({ from: process.env.EMAIL_FROM ?? "VOIDCOIN <onboarding@resend.dev>", to: email, subject: "VOIDCOIN moderator sign-in", html: `<p>This link expires in 15 minutes.</p><p><a href="${url}">Open the moderation chamber</a></p>` }, { headers: { "Idempotency-Key": `voidcoin-admin-${token.slice(0, 24)}` } });
