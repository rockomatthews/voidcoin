...
`VOIDGraduationExecutor` is the supported execution path whenever the pool or curve moves after seeding. It reads the active adapter, pool, current curve liquidity quote, and live Uniswap price within one transaction; determines which side must be traded; uses only the caller's bounded VOID approval or `msg.value`; places a SwapRouter02 trade with the exact target as its price limit; calls `graduate()`; then returns every unused input and swap output. A sub-part-per-billion sqrt-price tolerance treats only the seed caps' integer-flooring dust as already aligned; larger movement still requires correction. If the supplied correction is insufficient, the target is wrong, or final migration fails, the entire transaction reverts. No curve reserve is spent on price correction. A heavily capitalized hostile pool may still require the Safe's delayed migration-target replacement rather than an uneconomic correction.

The executor caches canonical Base WETH9 immutably at construction so WETH9's 2,300-gas `transfer` refund reaches a receive path with no external calls. Every replacement migration target must report that same WETH9 address or the executor fails closed. Keeper simulations should still provide conservative limits for both possible correction assets because the required direction is determined from live transaction state; an economically unnecessary asset is not pulled and unused ETH is refunded.
...
The locker has no owner or emergency withdrawal. It records the adapter that registered each NFT, and the curve independently verifies that the returned token ID is held and registered by its active migration target. Anyone may call `release(tokenId)` after the registered unlock time, but the NFT always transfers safely to the immutable Safe beneficiary. Because release uses `safeTransferFrom`, the production Safe must return the ERC-721 receiver selector through its configured fallback handler; deployment preflight rejects a Safe that does not. Safe owners must not remove or replace that receiver-capable fallback handler during the 365-day lock, and operations must verify it onchain periodically. Fees remain inside the position while it is locked.

## Verification

The normal Foundry suite includes the expert-delivered price-aware `UniV3Model.sol`, with real mint-ratio mechanics, exact-limit swaps, SwapRouter02 allowance consumption, and Base-style WETH9 refunds. It exercises ordering, both correction directions, seed rounding tolerance, price bounds, minimum asset use, dust routing, deflationary-token rejection, failed migration rollback, position registration, custody, and release timing.
