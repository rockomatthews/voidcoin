...
- Bonding-curve configuration is economic code. The virtual reserve controls price and slippage; the threshold controls migration eligibility. Graduation uses the official Base Uniswap v3 position manager and SwapRouter02, a fixed 1% pool fee, full-range ticks, a 99.9% minimum use requirement for both assets, and immutable 12-month NFT custody. The ratio-matched seed is the Safe gate. The atomic executor uses caller-funded bounded correction assets and refunds residuals; it cannot spend curve reserves. The adapter, executor, and locker still require independent semantic review even when live-state Base Mainnet-fork rehearsals pass.
