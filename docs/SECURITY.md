...
The caller cannot burn outside the live record range. `burnForRename` requires the expected next burn ID, at least 1,000,000 VOID for the first record, then the larger of `recordBurn + 250,000 VOID` and `recordBurn + 10%`, with a maximum strategic premium of 2,000,000 VOID above that floor. The exact amount becomes the new record and is commitment-bound. A stale ID, stale replacement, below-floor amount, or above-cap amount reverts before token transfer or commitment mutation.

`npm run contracts:security` is the canonical static-analysis command. It analyzes the Zora controller and the preserved V1/V2 production contracts with Slither, direct `solc`, and the exact 0.8.30 compiler. This avoids the unresolved inheritance references produced by the current Foundry build-info ingestion path. The command fails closed when the expected compiler or Slither is unavailable, and CI runs it on every push and pull request.

`VOIDLaunch` performs the genesis allocation atomically: 98% enters the continuous buyer-funded curve and 2% enters a non-transferable creator-beneficiary vesting contract. Vesting starts only after successful graduation and runs linearly for 12 months. The production Safe is installed directly as token and curve owner; the launch contract never owns either. The curve charges 1% in each direction, accounts reserves internally, remains open after reaching the threshold, and closes only after successful migration. The Safe authorizes graduation by seeding the active target; execution is then permissionless so a live-price correction and graduation can be atomic. Failed correction, burn, or migration calls roll back without closing trading.
...
- The Safe ERC-721 receiver check is a deployment-time snapshot. Safe owners must preserve a receiver-capable fallback handler and monitor it throughout the LP lock or `safeTransferFrom` release can be permanently stranded.
- The 1% curve fee made the tested sandwiches loss-making but does not eliminate griefing. The phase-three review measured victim shortfalls of 193–3,547 bps from attackers willing to lose money; callers must use fresh nonzero slippage limits and short deadlines.
...
- Bonding-curve configuration is economic code. The virtual reserve controls price and slippage; the threshold controls migration eligibility. Graduation uses the official Base Uniswap v3 position manager and SwapRouter02, a fixed 1% pool fee, full-range ticks, a 99.9% minimum use requirement for both assets, and immutable 12-month NFT custody. The ratio-matched seed is the Safe gate. The atomic executor caches canonical Base WETH9, accepts only a sub-part-per-billion rounding tolerance, uses caller-funded bounded correction assets, and refunds residuals; it cannot spend curve reserves. The adapter, executor, and locker still require independent semantic review even when live-state Base Mainnet-fork rehearsals pass.
