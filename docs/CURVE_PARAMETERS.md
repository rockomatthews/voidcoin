...
- Pool seed: 0.1% of the graduation-matched token amount and 0.1% of current accounted ETH for each Safe-approved migration target
...
- A loss-making transaction-ordering attacker can still worsen a victim's execution. The phase-three review measured 193–3,547 bps of victim shortfall across its tested griefing cases even though the attacker lost money. Every buy and sell must therefore use a fresh nonzero slippage bound and short deadline; the 1% fee is not an anti-MEV guarantee.
...
- The seed uses the same ETH/VOID ratio as the final migration. If curve trading or pool swaps change that ratio after seeding, the atomic graduation executor reads the live state, trades only caller-supplied correction assets up to caller-selected limits, graduates in the same transaction, and refunds unused inputs and swap output.
