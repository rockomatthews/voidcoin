...
- OpenZeppelin ERC-20, ownership, safe-transfer, and reentrancy dependencies pinned by the lockfiles.
...
- LP allocation: 70% in the tight opening range and 30% in a contiguous wide range, less atomically burned position-manager rounding dust.
- Creator allocation: 20,000,000 VOID in non-transferable vesting, with a 30-day cliff and full vesting after 365 days.
...
3. A predicted/preinitialized hostile pool can only revert deployment and cannot redirect assets; the contract address must remain private until confirmation.
...
11. Both positions can be released only after 365 days and only to the immutable Safe beneficiary; deployment must reject a Safe that cannot receive either NFT.
...
- QuickNode Base Mainnet fork block: `50146331`.
- An exact `1.000000 USDC` genesis swap through official Base `SwapRouter02` returned
  `1,000,334.564729308694295262 VOID`, enough for the initial `1,000,000 VOID` takeover.
...
- The token1 opening price is the exact `TickMath.getSqrtRatioAtTick(414600)` value; this is required for a
  genuinely one-sided mint and is covered by the price-aware position-manager test.
- A non-broadcast Base simulation also exercised the token1 ordering through the real position manager and
  completed the token-only LP mint successfully.
- A real Base-fork $100,000 purchase crossed the tight boundary, and a subsequent $1,000 purchase still returned VOID from the wide range.
